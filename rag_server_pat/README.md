...
This server is dedicated to searching and retrieving items from the official regional price list (Prezziario) for the Trento region. All price list files and data are sourced directly from the official website: [http://www.elencoprezzi2025.provincia.tn.it/](http://www.elencoprezzi2025.provincia.tn.it/).
...
1. **Document Preparation & Embedding (Offline/Batch):**
	- Source documents are parsed and chunked into activity blocks (output: `chunks.txt`).
	...
	- The upload script reads directly from `chunks.txt` and can upload all or a subset of chunks. Oversized chunks (metadata > 40kB) are automatically skipped.

Test the chunk size with following command:
```sh
python -c "with open('chunks_file.txt', 'r', encoding='utf-8') as f: \
...
**Embedding/Upload:**
1. Run `python rag_training.py` to encode and upload all chunks from `chunks.txt` to Pinecone.
	- The script will automatically skip any chunk whose metadata exceeds Pinecone's 40kB limit.
...
2. Install dependencies:
	...
	```
3. Create a `.env` file in `rag_server_pat/` with your Pinecone credentials:
	```
...
You can test the retrieval pipeline locally using the script, bypassing Pinecone, by setting `use_pinecone=False` in your retrieval function:
```python
results = embed_and_retrieve_pat(user_query, all_chunks_file="chunks.txt", top_k=1, use_pinecone=False)
```
...
def get_pinecone_index_dep():
	...
	return get_pinecone_index(index_name="pat-chunks")
...
---
For more details, see the code in `rag_training.py`, `routes.py`, and `pinecone` integration.