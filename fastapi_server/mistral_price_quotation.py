...
messages = [
  (
    "system",
    f"Based on the provided site visit timeline and description and bill of quantities, return only a valid JSON object as specified. Do not include any explanation, markdown, or commentary. Do not wrap the JSON in code blocks. Output only the JSON. Based on the book https://psu.pb.unizin.org/buildingconstructionmanagement/ and following standard: {activity_keywords}. Do site internal construction project cost planning and price quotation for the client created in right order of construction timeline, you should prepare object in JSON format in exact following structure with three keys: price_quotation: {boq_str} and internal_costs: {internal_costs_str}. You must evaluate the project as an expert construction engineer to prepare price quotation and internal costs official report where prices and costs are aligned and quotation calculated from internal costs, add all necessary construction works for the site in the correct order according to the timeline and European construction standards, calculate total price for each resource based on quantity, dimensions, unity of price, formula, if quantity or measures are not clear you should make estimations from provided site description. You should set average timeline how long the works takes and what find what are the costs, and estimate what is required according to the provided construction details, compare the output information with the market if the prices and timeline is reasonable. Logistics includes cost of the gas from vehicle in working area and the hotel if workers are far away from starting point and meals for the workers when staying overnight. You must include it in overhead costs. If the prices are not provided you should search only from the trusted sources like PAT Prezziario http://www.elencoprezzi2025.provincia.tn.it. You should update and include timeline. The original site information with timeline, site description and draft bill of quantities is following:",
  ),
  ("human", "{input}"),
]
