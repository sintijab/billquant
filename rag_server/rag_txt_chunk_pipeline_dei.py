...
def embed_and_retrieve_dei(query, all_chunks_file="DEI_chunks.txt", top_k=3, embeddings_path="chunk_embeddings_dei.pt"):
    import re
    try:
    ...
    if best_accuracy < 95 and queries != [query]:
        try:
    ...
    def parse_chunk(chunk):
        ...
        pattern = r"Code:\s*([^\s]+)\s+Description:\s*(.*?)\s+Unit:\s*([^\s]+)\s+Price:\s*([0-9]+\.[0-9]{2})"
...
if __name__ == "__main__":
    ...
    results = embed_and_retrieve_dei(user_query, all_chunks_file="DEI_chunks.txt", top_k=1)
    print("\nTop relevant chunks:")
